...
class Metric_List(View):
    ...
    def get(self, request):
        ...
        self.__context.push({
            "page_title": _("Metrics · %s") % self.__context.get("app_name", os.getenv("APP_NAME", "Badger"))
        })
...
class Metric_Add(View):
    ...
    def get(self, request):
        ...
        self.__context.autoload_user(request.user.id if request.user.is_authenticated else None)
        self.__context.push({
            "page_title": _("Add a Metric · %s") % self.__context.get("app_name", os.getenv("APP_NAME", "Badger"))
        })
...
class Metric_Edit(View):
    ...
    def get(self, request, metric_id):
        ...
        self.__context.autoload_user(request.user.id if request.user.is_authenticated else None)
        self.__context.push({
            "page_title": _("Edit Metric · %s") % self.__context.get("app_name", os.getenv("APP_NAME", "Badger")),
            "metric": metric
        })
